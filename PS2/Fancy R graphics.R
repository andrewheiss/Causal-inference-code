...
# TODO: This is so hacky, just copying/pasting the whole function. Move the plotting function out someday so it's more DRY.
ologit.points <- function(model, X, y.range, y.plot.label, legend.title, cat.labels, x.labels, x.plot.label, runs=500, y.limit=c(0, 0.75), plot.title=NULL) {
  library(ggplot2)
  library(reshape2)
  library(plyr)
  library(scales)
  ...
  p <- ggplot()
  p <- p + #geom_line(aes(x=xvar, y=value, group=category, colour=category), data=single.plot, size=2) + 
    geom_point(aes(x=xvar, y=value, group=interaction(simulation, category), colour=category), alpha=0.4, data=plot.data, size=3, position="jitter") + 
    scale_colour_brewer(palette="Dark2") +
    #     scale_colour_manual(values = rev(brewer.pal(5,"RdYlGn"))) + 
    #     scale_colour_manual(values = c("#2C7BB6", "#ABD9E9", "#FDAE61", "#D7191C")) + 
    guides(colour = guide_legend(override.aes = list(alpha = 1, size=2))) + 
    ...
    #     theme(text=element_text(family="Source Sans Pro", size=13)) + 
    theme(plot.background=element_rect(fill=NA, colour=NA), legend.position="top", legend.key=element_blank(), legend.background=element_rect(fill=NA, colour=NA)) + 
    guides(colour=guide_legend(override.aes=list(size=4, alpha=1)))  # Make legend dots bigger and not transparent
  p
}
